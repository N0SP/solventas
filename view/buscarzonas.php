...
$result = $conexion->query(
    ...
    "SELECT Id_Zona_Aeronaval, Id_Region, Nombre_Zona FROM zonas_aeronaval WHERE Id_Region = $seleccion"
);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $html .= '<option value="' . $row['Id_Zona_Aeronaval'] . '">' . $row['Nombre_Zona'] . '</option>';
    }
}
