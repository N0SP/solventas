...
//        $.get("./ajax/IngresoAjax.php?op=listTipo_Marca", function(r) {
//                $("#cboMarca").html(r);
//                console.log("Aqui");
//                console.log($("#cboMarca"));
    ...
    //  var marca = document.getElementsByName("cboMarca");
        ...
        for (var i = 0; i < stock_ing.length; i++) {
            if (stock_ing[i].value !== "") {
                AgregarDetalleRegistrar(serie[i].value, desc[i].value, marca[i].value, modelo[i].value, color[i].value,
                     stock_ing[i].value, 1, prec_comp[i].value);
            }
        }
...
/*		
    ...
    */        
    ...
    function AgregarDetalle(serie, descripcion, marca, modelo, color, stock_ing, p_compra) {
        var detalles = new Array(serie, descripcion, marca, modelo, color, stock_ing, p_compra);
        elementos.push(detalles);
        ConsultarDetalles();
    }

    function AgregarDetalleRegistrar(serie, descripcion, marca, modelo, color, stock_ing, p_compra) {
        var detallesReg = new Array(serie, descripcion, marca, modelo, color, stock_ing, p_compra);
...
function ConsultarDetalles() {
        ...
        for (var pos in data) {
   //         $("table#tblDetalleIngreso").append("<tr><td>" + data[pos][1] + " <input class='form-control' type='hidden' name='txtIdArticulo' id='txtIdArticulo[]' value='" + data[pos][0] + "' /></td><td><input class='form-control' type='text' onkeyup='Modificar(" + pos + ");' name='txtCodgo' id='txtCodgo[]' value='" + data[pos][2] + "' /></td><td><input class='form-control' type='text' name='txtSeries' onkeyup='Modificar(" + pos + ");' id='txtSeries[]'  value='" + data[pos][3] + "' /></td>
  
    //PRUEBA

    var registro = "<tr>";
 
    registro += data[pos][0] + "<td> <input class='form-control' type='text' name='txtSeries' id='txtIdSeries[]' value='" + data[pos][0] + "' /></td>";
    registro += data[pos][1] + "<td><input class='form-control' type='text' name='txtDescripcion' onkeyup='Modificar(" + pos + ");' id='txtDescripcion[]' value='" + data[pos][1] + "'  /> </td>";
  /* 
    $.get("./ajax/IngresoAjax.php?op=listTipo_Marca", function(r) {
  ...
  */    
 //   registro += data[pos][2] + "<td><input class=¨form-control¨ type=¨text¨ name='txtMarca' onkeyup='Modificar(" + pos + ");' id='txtMarca[]' value='" + data[pos][2] + "' /></td>";
  //  registro += data[pos][2] + "<td><select class='form-control' name='cboMarca[]' id='cboMarca[]'><option value='0'>Seleccione:</option>  onkeyup='Modificar(" + pos + "); value='"  + data[pos][2] +  "' /></select> </td>";  
    registro += data[pos][3] + "<td><input class='form-control' type='text' name='txtModelo' onkeyup='Modificar(" + pos + "); id='txtModelo[]' value='" + data[pos][3] + "' /></td>";
    registro += data[pos][4] + "<td><input class='form-control' type='text' name='txtColor' onkeyup='Modificar(" + pos + ");' id='txtColor[]' value='" + data[pos][4] + "' /></td>";
    registro += data[pos][5] + "<td><input class='form-control' type='text' onkeypress='return justNumbers(event);' name='txtStockIng' id='txtStockIng[]' value='" + data[pos][5] + "'  onkeyup='calcularTotal(" + pos + ");' required /> </td>";
    registro += data[pos][6] + "<td><input class='form-control' type='text' onkeypress='return onKeyDecimal(event,this);' name='txtPrecioComp' id='txtPrecioComp[]' value='" + data[pos][6] + "'  onkeyup='calcularTotal(" + pos + ");' required /></td>";    
    registro += data[pos][7] + "<td WIDTH=table#tblDetalleIngreso'100'><button type='button' data-toggle='tooltip' title='Quitar Articulo del detalle' onclick='eliminarDetalle(" + pos + ")' class='btn btn-danger'><i class='fa fa-remove' ></i> </button> </td>";  
    
//    if (valueLinea.error) {
          registro += "</tr>";
          $("table#tblDetalleIngreso tbody").append(registro);
...
function ListadoIngresos(){ 
  ...
  //      var marca = document.getElementsByName("cboMarca");
        ...
        elementos[pos][0] = serie[pos].value;
        elementos[pos][1] = desc[pos].value;
        elementos[pos][2] = marca[pos].value;
        elementos[pos][3] = modelo[pos].value;
        elementos[pos][4] = color[pos].value;
    ...
    function AgregarDetalles(id1, id2, id3, id4, id5, id6, id7){
        alert(id1 + " "+ id2 + " " + id3 + " " + id4 + " "+ id5 + " "+ id6 + " " + id7);
    }

    function AgregarDetalleCarrito(serie, descripcion, marca, modelo, color, stock_ing,  p_compra) {
        var detalles = new Array(serie, descripcion, marca, modelo, color, stock_ing, p_compra);
        elementos.push(detalles);
        ConsultarDetalles();
    }
